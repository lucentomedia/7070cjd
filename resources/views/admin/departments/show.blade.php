...
@section('title', $dept->title.' Allocations &amp; Tasks')
@section('page_title') <i class="fa fa-th-large mr10"></i>{{$dept->title}} @endSection
...
<ol class="breadcrumb float-right nav_breadcrumb_top_align" style="margin-bottom: 0">
	...
	<li class="breadcrumb-item active h-padding-5 no-right-padding">{{$dept->title}}</li>
</ol>
@endsection
...
@section('content')
	...
	<div class="row">
		...
		<div class="col-sm-8">
			...
			<div class="card">
				...
				<div class="card-header bgc-070">
					<h4 class="font-600 text-center no-padding no-margin text-uppercase c-fff">{{$dept->title}} Allocations</h4>
				</div>
				...
				<div class="card-block">
					...
					@if ($alls->count() == 0)
						...
						<p class="alert alert-info">There are no allocations to this department.</p>
	...
	<div class="card mt40">
		...
		<div class="card-header bgc-070">
			<h4 class="font-600 text-center no-padding no-margin text-uppercase c-fff">{{$dept->title}} Tickets</h4>
		</div>
